...
-- Create policies for store productions with proper store access
...
-- Create policies for production items with proper store access
...
-- Create policies for box productions with proper store access
...
-- Update sync_user_role function to properly handle store_ids
CREATE OR REPLACE FUNCTION sync_user_role()
RETURNS TRIGGER AS $$
BEGIN
  -- Update user metadata with proper store_ids array
  UPDATE auth.users
  ...
  -- Force a session refresh by updating the last_sign_in_at
  UPDATE auth.users
  SET last_sign_in_at = now()
  WHERE id = NEW.user_id;
  
  -- Force a session refresh for all active sessions
