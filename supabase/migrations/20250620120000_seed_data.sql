...
VALUES
  (gen_random_uuid(), 'Original Glazed', 'Signature glazed ring', (SELECT id FROM donut_forms WHERE name='Ring' ORDER BY id LIMIT 1), 0.35, true),
  (gen_random_uuid(), 'Chocolate Iced', 'Ring with chocolate icing', (SELECT id FROM donut_forms WHERE name='Ring' ORDER BY id LIMIT 1), 0.40, true),
  (gen_random_uuid(), 'Strawberry Sprinkles', 'Ring with strawberry icing & sprinkles', (SELECT id FROM donut_forms WHERE name='Ring' ORDER BY id LIMIT 1), 0.42, true),
  (gen_random_uuid(), 'Custard Filled', 'Filled with smooth custard', (SELECT id FROM donut_forms WHERE name='Filled' ORDER BY id LIMIT 1), 0.50, true),
  (gen_random_uuid(), 'Raspberry Filled', 'Filled with raspberry jam', (SELECT id FROM donut_forms WHERE name='Filled' ORDER BY id LIMIT 1), 0.50, true),
  (gen_random_uuid(), 'Cinnamon Cake', 'Cinnamon-coated cake doughnut', (SELECT id FROM donut_forms WHERE name='Cake' ORDER BY id LIMIT 1), 0.38, true)
ON CONFLICT DO NOTHING;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM production_plans WHERE date::date = CURRENT_DATE) THEN
    INSERT INTO production_plans (id, date, created_by, total_production, status)
    VALUES (gen_random_uuid(), CURRENT_DATE::text, (SELECT id FROM auth.users LIMIT 1), 0, 'draft')
