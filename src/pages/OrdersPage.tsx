...
const OrdersPage: React.FC = () => {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      setOrdersError(
        error instanceof Error ? error.message : 'Impossible de charger les commandes depuis Supabase.'
      );
    } finally {
  ...
  const handleRemoveItem = (index: number) => {
    setForm((prev) =>
      prev ? { ...prev, items: prev.items.filter((_, itemIndex) => itemIndex !== index) } : prev
    );
  };
  ...
  const validateForm = () => {
    ...
    if (form.items.some((item) => item.quantity <= 0)) {
      errors.push('Chaque ligne doit avoir une quantité positive.');
    }
  ...
  const handleProductionFieldChange = (orderId: string, value: string) => {
    setOrders((prev) =>
      prev.map((order) => (order.id === orderId ? { ...order, productionDate: value } : order))
    );
  };
  ...
  const saveAdminDate = async (orderId: string) => {
    ...
    try {
      ...
      const updated = await updateOrderProduction(
        orderId,
      ...
      setOrdersError(
        error instanceof Error
          ? error.message
          : "Impossible d'enregistrer la date de production."
      );
    } finally {
  ...
  const approveOrder = async (orderId: string) => {
    ...
    try {
      ...
      setOrders((prev) => prev.map((item) => (item.id === orderId ? updated : item)));
      setSuccessMessage(
        'Commande validée. Elle doit maintenant apparaître dans le plan du bon magasin.'
      );
    } catch (error) {
  ...
  const handleDeleteOrder = async (orderId: string) => {
    ...
    const confirmed = window.confirm(
      `Supprimer la commande de ${order.customerName} pour ${
    ...
    <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <select
                    ...
                    onChange={(event) =>
                      handleFormChange('orderType', event.target.value as OrderType)
                    }
                    className="rounded-md border-gray-300 shadow-sm focus:border-krispy-green focus:ring-krispy-green"
                  >
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-gray-500">
                  La date de production reste réservée à la validation admin.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-krispy-green focus:ring-krispy-green"
                    >
                      {conditioningOptions.map((option) => (
                        <option key={option} value={option}>
                          {formatConditioningLabel(option)}
                        </option>
                      ))}
                    </select>
                  </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Sélection depuis le catalogue actif.
                  </p>
                </div>
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium rounded-md text-white bg-krispy-green hover:bg-krispy-green-dark"
                >
                  Ajouter une ligne
                </button>
              </div>
                ...
                >
                  <div className="md:col-span-5">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        handleItemChange(index, 'varietyId', event.target.value)
                      }
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-krispy-green focus:ring-krispy-green"
                    >
                  ...
                  <div className="md:col-span-3">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        handleItemChange(index, 'conditioning', event.target.value)
                      }
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-krispy-green focus:ring-krispy-green"
                    >
                      {conditioningOptions.map((option) => (
      ...
      <div className="bg-white shadow rounded-lg p-6 border border-gray-100">
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-600">
            Les magasins ne voient que leurs propres commandes.
          </p>
        </div>
                            ...
                            {!order.productionApproved && (
                              <div className="space-y-2">
                                <input
                                  ...
                                  onChange={(event) =>
                                    handleProductionFieldChange(order.id, event.target.value)
                                  }
                                  className="rounded-md border-gray-300 shadow-sm focus:border-krispy-green focus:ring-krispy-green"
                                />
                                <button
                      ...
                      <td data-label="Articles" className="px-4 py-4 text-sm text-gray-900">
                        <ul className="space-y-1">
                          {order.items.map((item, itemIndex) => {
                            const variety = catalogue.find(
                              (entry) => entry.id === item.varietyId
                            );

                            return (
                              <li
                                key={`${order.id}-${item.varietyId}-${itemIndex}`}
                                className="flex flex-col"
                              >
                                <span className="font-medium">
                                  {variety?.name || item.varietyId}
                                </span>
                                <span className="text-gray-600 text-xs">
                                  {item.quantity} x {formatConditioningLabel(item.conditioning)}
                                </span>
                              </li>
                            );
                          })}
                      ...
                      <td data-label="Actions" className="px-4 py-4 text-right text-sm">
                        <div className="flex flex-col gap-2 items-stretch md:items-end">
                          ...
                          {canManageOrders && order.productionApproved && (
                            <span className="text-green-700 text-xs font-medium md:text-right">
                              Validation terminée
                            </span>
                          )}
                          ...
                          {canDeleteOrder(order) && (
                            <button
                              ...
                              disabled={isSavingThisOrder}
                              className="inline-flex items-center px-3 py-2 text-xs font-medium rounded-md border border-red-200 text-red-700 hover:bg-red-50 disabled:opacity-60"
                            >
