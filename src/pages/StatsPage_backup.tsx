...
import { BarChart2, PieChart, TrendingUp, DollarSign, Store, Coffee, ShoppingBag, Printer } from 'lucide-react';
...
const StatsPage: React.FC = () => {
  ...
  // Cost calculations
  const costPerDonut = 0.50; // Cost in CHF
        ...
        plan.stores.forEach((store: any) => {
          ...
          if (store.production_items && Array.isArray(store.production_items)) {
            store.production_items.forEach((item: any) => {
              ...
              if (waste > 0) {
                console.log(`🗑️ Variety Waste Cost: ${variety?.name || 'Unknown'} - ${waste} waste × CHF ${varietyCost} = CHF ${itemWasteCost.toFixed(2)}`);
              }
            });
          }
          ...
          if (store.box_productions && Array.isArray(store.box_productions)) {
            store.box_productions.forEach((boxProd: any) => {
              ...
              if (box) {
                ...
                if (wasteBoxes > 0) {
                  console.log(`🗑️ Box Waste Cost: ${box.name} - ${wasteBoxes} boxes wasted × CHF ${boxUnitCost.toFixed(2)}/box = CHF ${totalBoxWasteCost.toFixed(2)}`);
                }
              }
            });
          }
  ...
  const generateSalesReport = () => {
    ...
    // Calculate production total for cost calculation (like in UI)
    const totalProductionForCost = storePerformance.reduce((sum, store) => sum + store.production, 0);
    
    const overallWastePercent = totals.received > 0 ? (totals.waste / totals.received) * 100 : 0;
    const productionCost = totalProductionForCost * 0.50; // CHF 0.50 per donut - use planned production like UI
              ...
              <div className="text-sm text-gray-500 space-y-1">
                ...
                <div className="flex items-center text-xs">
                  {performanceComparison.waste.isIncrease ? (
                    <span className="text-red-600">↑ {Math.abs(performanceComparison.waste.changePercent).toFixed(1)}%</span>
                  ) : (
                    <span className="text-green-600">↓ {Math.abs(performanceComparison.waste.changePercent).toFixed(1)}%</span>
                  )}
              ...
              <div className="text-sm text-gray-500 space-y-1">
                ...
                <div className="flex items-center text-xs">
                  {performanceComparison.production.isIncrease ? (
                    <span className="text-blue-600">↑ {Math.abs(performanceComparison.production.changePercent).toFixed(1)}%</span>
                  ) : (
                    <span className="text-green-600">↓ {Math.abs(performanceComparison.production.changePercent).toFixed(1)}%</span>
                  )}
                  <span className="ml-1">vs période précédente</span>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-800 flex items-center">
                <Coffee className="h-5 w-5 mr-2 text-krispy-green" />
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-800 flex items-center">
                <ShoppingBag className="h-5 w-5 mr-2 text-krispy-green" />
