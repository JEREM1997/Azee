...
export const buildArticlePdf=(args:{rows:ArticleRow[];series:ArticleSeries[];totals:ArticleTotals;article:string;type:string},ctx:ReportContext)=>{
  ...
  const cards=[['PRÉVU / PRODUIT',formatPdfNumber(args.totals.planned)],['RÉCEPTIONNÉ',formatPdfNumber(args.totals.received)],['VENDU',formatPdfNumber(args.totals.sold)],['DÉCHETS',formatPdfNumber(args.totals.waste)],['TAUX DÉCHETS',formatPdfPercent(args.totals.wasteRate)],['ÉCART RÉCEPTION',formatPdfNumber(args.totals.receptionGap)]];
  const width=(doc.internal.pageSize.getWidth()-PDF_MARGIN*2-15)/6;cards.forEach(([label,value],i)=>addKpiCard(doc,{x:PDF_MARGIN+i*(width+3),y,width,label,value}));y+=34;
  ...
  addDataTable(doc,{y,head:[['Production','Livraison / vente','Magasin','Article','Prévu','Reçu','Déchets','Vendu','Taux','Statut']],body:args.rows.map(r=>[formatPdfDate(r.productionDate),formatPdfDate(r.salesDate),r.storeName,r.productName,formatPdfNumber(r.planned),formatPdfNumber(r.received),formatPdfNumber(r.waste),formatPdfNumber(r.sold),r.received!==null&&r.received>0&&r.waste!==null?formatPdfPercent(r.waste/r.received):'—',r.status]),compact:true,columnStyles:{0:{cellWidth:22},1:{cellWidth:25},2:{cellWidth:33},3:{cellWidth:45},4:{cellWidth:17,halign:'right'},5:{cellWidth:17,halign:'right'},6:{cellWidth:17,halign:'right'},7:{cellWidth:17,halign:'right'},8:{cellWidth:18,halign:'right'},9:{cellWidth:37}}});
