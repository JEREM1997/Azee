...
CREATE POLICY "Allow read access to authenticated users for stores"
  ON stores
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Allow all access to admin users for stores"
  ON stores
  TO authenticated
...
CREATE POLICY "Allow read access to authenticated users for varieties"
  ON donut_varieties
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Allow all access to admin users for varieties"
  ON donut_varieties
  TO authenticated
  USING (auth.jwt() ->> 'role' = 'admin')
  WITH CHECK (auth.jwt() ->> 'role' = 'admin');
...
CREATE POLICY "Allow read access to authenticated users for forms"
  ON donut_forms
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Allow all access to admin users for forms"
  ON donut_forms
  TO authenticated
  USING (auth.jwt() ->> 'role' = 'admin')
  WITH CHECK (auth.jwt() ->> 'role' = 'admin');
...
CREATE POLICY "Allow read access to authenticated users for boxes"
  ON box_configurations
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Allow all access to admin users for boxes"
  ON box_configurations
  TO authenticated
  USING (auth.jwt() ->> 'role' = 'admin')
  WITH CHECK (auth.jwt() ->> 'role' = 'admin');
